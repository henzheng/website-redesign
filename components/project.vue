<template>
    <div class="flex flex-col w-[90vw] items-center md:items-start md:w-auto md:flex-row gap-4">
        ...
        <div class="flex flex-col w-[80%] md:w-1/2">
            <h2 class="font-sentient text-lg">{{ projectName }}</h2>
            <p class="font-sentient">{{ projectRole }}</p>
            <div class="bg-slate-800 opacity-80 rounded-sm text-white p-3 my-4">
                <p class="text-white font-sentient text-sm">{{ description }}</p>
            </div>
            <ul class="flex flex-row gap-4">
                <li class="font-satoshi text-black px-2 py-1 rounded-md" :class="`bg-${tech.technologyColor}-300`"
